...
export default function checkQuickswap(address,signer,provider) {
    ...
    addressQuickswap = "0x6c28AeF8977c9B773996d0e8376d2EE379446F2f" ;
    
    const [ hasToken1, sethasToken1 ] = React.useState(false);
    const [ amtToken1, setamtToken1 ] = React.useState(0);

    const quickContract = new ethers.Contract(addressQuickswap, abi, signer);
    var balance = await quickContract.balanceOf(address.toString()); 
    var decimals = await quickContract.decimals();
    balance1 = ethers.utils.formatEther(balance)

    if( balance1 != 0  ){
        sethasToken1 = true;
        setamtToekn1 = balance1;
    }
    return amtToken1 ;
}
